...
class TextMelDataset(torch.utils.data.Dataset):
    def __init__(self, filelist_path, cmudict_path, add_blank=True,
        ...
        print(f"filepaths_and_text {self.filepaths_and_text}")
...
class TextMelSpeakerDataset(torch.utils.data.Dataset):
    def __init__(self, filelist_path, cmudict_path, add_blank=True,
        ...
        # print(f"filepaths_and_text {self.filelist}")
        self.cmudict = cmudict.CMUDict(cmudict_path)
        self.n_fft = n_fft
        self.n_mels = n_mels
        self.sample_rate = sample_rate
    ...
    def get_mel(self, filepath):
        ...
        if sr != self.sample_rate:
            print(f"Skipping file with sample rate {sr} (expected {self.sample_rate}): {filepath}")
            return None  # Return None if the sample rate doesn't match
        # assert sr == self.sample_rate
        mel = mel_spectrogram(audio, self.n_fft, self.n_mels, self.sample_rate, self.hop_length,
                              self.win_length, self.f_min, self.f_max, center=False).squeeze()
        return mel
    ...
    def __getitem__(self, index):
        while True:  # Keep looping until a valid sample is found
            text, mel, speaker = self.get_triplet(self.filelist[index])
            if mel is not None:  # Skip samples with invalid mel
                break
            # Choose a new random index to avoid invalid samples
